<template>
  <Dialog v-model:visible="showDialog" modal header="从Bangumi中添加..." :draggable="false" class="bgm_search_dialog_content" :closable="true">
    <div class="flex items-center gap-2 mb-4">
      <InputText size="small" class="flex-auto" autocomplete="off" v-model="title" placeholder="搜索标题" @keyup.enter="searchBangumi(title)" autofocus />
    ...
    <div>
      <DataTable :value="ls" v-if="ls.length!=0" class="mb-5" stripedRows tableStyle="table-layout: fixed; width: 100%">
        ...
        <Column header="操作" style="width: 140px;" class="select-none" v-if="!mobile">
          <template #body="slotProps">
